...
while True:
    ...
    ras.moving()
    rocks.manage_rock(ras.curr_x,ras.curr_y, game.stagenum)
    ...
    #all window is clean, set next stage and initialize game
    if len(windows.list) == 0:
        game.change_stage(setting.draw, setting.image, setting.disp)
        berry, ras, windows, rocks = game.gameinit()
        continue

    #if it is last stage, end game
    elif game.stagenum ==3:
