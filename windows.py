...
class Windows():
    ...
    def __init__(self):
        ...
        #List for saving broken window objects
        self.list = []
    ...
    def window_check(self,x,y):
        """
        if strawberry is on the window, window clean
        and remove completely fixed window from the list
        """
        for window in self.list:
            #check foe overlap window and strawberry
            if abs((window.curr_x+5) - (x+20))<10 and abs((window.curr_y+10) - (y+20))<5:
                window.clean()

        #left broken window in list
        self.list = [window for window in self.list if window.statenum <5]
